...
export const apiMethods = {
    ...
    // fieldIdCounter starts at 1 for every fresh FormBuilder instance
    // (constructor), regardless of what fields an existing form already
    // has. Without this, editing an existing form and dropping a new field
    // can regenerate a name that's already used by one of its persisted
    // fields (e.g. "text_1"), which the server rejects with a UNIQUE
    // constraint failure on (form_definition_id, field_name) - reproduced
    // by loading an existing form with a "text_1" field and dropping
    // another text field. Called after loading fields from an existing
    // form or a template, so newly-generated names never collide with
    // fields that already came from the server.
    updateFieldIdCounterFromFields(fields) {
    ...
    async loadForm() {
        try {
            ...
            this.updateFieldIdCounterFromFields(this.fields);
