/**
 * Field property editor: the tabbed modal (Basic / Conditional Logic /
 * Validation / Dependencies) shown when adding or editing a field, and
 * saveFieldProperties() which reads the DOM state of all four tabs back off
 * that same modal when it's saved.
 *
 * Mixed onto FormBuilder.prototype in form-builder.js (Object.assign), not a
 * standalone class of its own - these methods read/write
 * `this.fields`/`this.currentFieldIndex`/`this.isNewField`/`this.config`,
 * and editField()/saveFieldProperties() call back into
 * deleteFieldSilently()/renderCanvas()/updatePreview(), which still live on
 * the single FormBuilder instance.
 */
export const fieldPropertyMethods = {
